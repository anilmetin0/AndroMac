...
AndroMac keeps an Android phone and a Mac in sync over your own Wi-Fi network. It carries the
...
`AndroMac-<version>-macOS-arm64.dmg` for the Mac and `AndroMac-<version>-android.apk` for the
phone, with their checksums in `SHA256SUMS.txt`. The Mac build is Apple Silicon only (M1 and
later); there is no Intel build. The APK is one file for every phone: the app has no native code,
so there is nothing to pick between arm64 and the rest.

### macOS 14 Sonoma or later

1. Open the DMG and drag `AndroMac.app` to **Applications**.
2. The app is signed ad-hoc and is not notarized, so macOS blocks the first launch. Open it
   once, let it be refused, then go to **System Settings → Privacy & Security** and press
   **Open Anyway**. From the terminal instead:
...
2. Open AndroMac. It asks for the notification permission immediately and offers the
...
5. Once everything is granted the card reads "All permissions granted". The optional
   permissions stay listed on the Permissions screen until you grant them.

[Obtainium](https://github.com/ImranR98/Obtainium) installs and updates the APK directly from
...
keys on the tag name, which stays the same while a version is in development. It will not
...
<table>
<tr>
<td align="center"><img src="docs/images/android-home.png" alt="The Android main screen" width="230"></td>
<td align="center"><img src="docs/images/android-home-dark.png" alt="The Android main screen in dark mode" width="230"></td>
<td align="center"><img src="docs/images/android-permissions.png" alt="The Permissions screen on Android" width="230"></td>
<td align="center"><img src="docs/images/android-connection.png" alt="The Connection screen on Android" width="230"></td>
</tr>
<tr>
<td colspan="2" align="center"><img src="docs/images/android-pairing.png" alt="The pairing code on Android" width="230"></td>
<td colspan="2" align="center"><img src="docs/images/macos-pairing.png" alt="The pairing code window on macOS" width="380"></td>
</tr>
<tr>
<td colspan="4" align="center"><img src="docs/images/macos-window.png" alt="The AndroMac main window on macOS" width="640"></td>
</tr>
<tr>
<td colspan="4" align="center"><img src="docs/images/macos-settings.png" alt="Settings on macOS, General" width="640"></td>
</tr>
<tr>
<td colspan="4" align="center"><img src="docs/images/macos-settings-sync.png" alt="Settings on macOS, Sync" width="640"></td>
</tr>
<tr>
<td colspan="4" align="center"><img src="docs/images/macos-settings-permissions.png" alt="Settings on macOS, Permissions" width="640"></td>
</tr>
</table>
...
temporary one and says so in the panel, so your pairing stays intact.
...
title `AndroMac 1.0.0`, and the assets carry the version in their names; the commit is the tag's
target. The notes are the `## <version>` section of `CHANGELOG.md`, with the matching section of
...
|---|---|
| The phone says the Mac was not found | AndroMac is open on the Mac; both devices are on the same Wi-Fi, not a guest network and not one with AP isolation; the Local Network permission was granted, reachable from Settings → Network or Settings → Permissions on the Mac. |
...
| The clipboard does not arrive from the phone by itself | That is the Android restriction described above. Opening the Mac panel asks the phone for it, and opening AndroMac on the phone sends it. For an instant answer grant **Display over other apps** on the phone; otherwise tap the notification it posts, the tile, or the share sheet. |
