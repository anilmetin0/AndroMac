...
its ringer and volume, and a button that makes a lost phone ring. There is no server, no account, no cloud and no
...
| **Clipboard** | Phone to Mac | The Mac asks when you open the panel, and the phone answers — or you push it yourself with the Quick Settings tile, the button on the ongoing notification, or the share sheet. See [why this direction is asked for](#why-the-clipboard-is-asked-for-in-one-direction). |
...
| **Phone controls** | Mac to phone | Ringer (ring, vibrate, silent) and media volume from the panel, plus a test notification that travels the whole mirroring chain and comes back. Silencing needs Do Not Disturb access on the phone; without it the button is disabled rather than silently failing. |
...
| **Several phones** | Mac | More than one Android can be paired and connected at once. The panel lists every device and expands on click; each one can be disconnected and connected on its own — Disconnect hangs up as well as refusing the next attempt — and each has its own clipboard switch. Forgetting a device is in Settings → Devices. |
...
deliberate privacy decision with no supported way around it, so the phone never pushes a copy by
itself. It answers a question instead: opening the Mac panel asks the phone for its clipboard, and
the phone reads it through an invisible activity that holds focus for a moment.

That activity needs one permission on the phone, **Display over other apps** — the single
exemption an ordinary app has from Android's background-activity rule. Without it the phone posts
a notification with one "Send clipboard" button, and the tile and the share sheet still work; the
share sheet route never touches the clipboard at all, which makes it the cleanest of them. A phone
that grants neither answers nothing, and nothing in the protocol can read its clipboard anyway.
...
2. Open AndroMac. It asks for the notification permission straight away and offers the
   notification access screen once, with the reason. The **Setup** section lists whatever is
   still missing:
...
3. While either of the first two is missing, a banner sits at the top of the screen saying which —
   tapping it opens the right settings screen, and the cross dismisses it until something else is
   revoked. A mirror that quietly relays nothing is the one state worth interrupting for.
...
5. The Setup section disappears by itself once everything is granted; the two optional rows stay
   until you grant them or decide you do not want them.
...
panel is opened, never on a timer. When a newer build exists — a greater version, or the same
version built from a different commit — the app offers it once, as `1.0.0 (fd7d47a)`, with
...
The check is the one thing that makes either app talk to a server — one request to
api.github.com carrying the app version and nothing else. It is on by default because an app
distributed outside any store has no other way to tell you a fix exists; the switch turns it off,
and then nothing leaves your network at all. See [Privacy and security](#privacy-and-security).
...
On the Mac the menu bar panel is for glancing. It shows, in order, a card with every paired phone —
each row expandable, carrying that phone's track, its volume slider and ringer buttons, its
clipboard switch, a ring button, a test-notification button and Disconnect — with battery
underneath, then the last clipboard entry with a progress row while a file is moving in either
direction, and the four most recent notifications. A status line appears above all of it only when
nothing is paired yet or the listener is down. Dropping files onto the panel sends them. What
syncs at all is decided once, in Settings, rather than at every glance; the detail lives in the
window.
...
| Settings | General, What syncs, Clipboard, Files, Updates, Devices, Network, Metrics and Privacy. |
...
Changing the language shows a Restart button, because the language is read at launch. What syncs
holds the four switches for **Battery**, **Clipboard**, **Notifications** and **Media**. Updates
holds the update check described under [Update check](#update-check). Files has
**Receive files** and **Accept files automatically**, the latter for every paired phone. Clipboard
holds the sync switch, the **Mac to phone** choice between automatic and manual, the concealed
clipboard rule, and whether opening the panel asks the phone for its clipboard. Updates also draws a QR code for the releases page, so the APK can be installed on
...
telemetry and no analytics. The apps open no socket to the internet, with one exception: the
update check in Settings → Updates, on by default, which asks `api.github.com` for the newest
release at most once a day and sends nothing but the app's version in the `User-Agent` header.
Installing an update downloads from `github.com` as well, and only after you press the button.
Turn the switch off and both apps go back to speaking to nothing but each other.
...
android/                           AGP 9.4.0, Gradle 9.7.1, minSdk 29, no dependencies
  ...
  app/src/main/kotlin/dev/andromac/
    ...
    feature/UpdateCheck.kt         the daily release check against the GitHub API
    feature/Updater.kt             downloads and verifies the APK, then the system installs it
    feature/SystemBridge.kt        ringer and volume, reported and settable from the Mac
    ...
    ui/UpdateSettingsActivity.kt   the update switch, Check now and the in-app install
...
macos/                             Swift package, swift-tools 6.2, macOS 14+, no dependencies
  ...
  Sources/AndroMac/
    ...
    UpdateCheck.swift              the daily release check against the GitHub API
    Updater.swift                  downloads, verifies and installs a release, then restarts
    FileTransfer.swift             file send and receive: offer, consent window, chunks, hash, quarantine
    MenuPanel.swift                the menu bar panel
    DeviceList.swift               every paired phone, expandable, with its controls and Disconnect
...
| The clipboard does not arrive from the phone by itself | That is the Android restriction described above. Opening the Mac panel asks the phone for it; for an instant answer grant **Display over other apps** on the phone, otherwise tap the notification it posts, the tile, or the share sheet. |
