...
This is a spare-time project with no security team behind it, so treat those as targets. If a
report is still unfixed at 90 days, publishing it is your call and there will be no complaint
about it.
...
- The protocol. The handshake, key agreement, key derivation, framing, nonce handling, replay
  protection, and the length and type limits the receiver enforces. See
  [docs/PROTOCOL.md](docs/PROTOCOL.md).
- Pairing and trust. The SAS comparison, key pinning, anything that would let a third device be
  pinned without the user seeing it, take over an established session, or downgrade a pinned key
  back to first contact.
- Storage. The identity key in the macOS Keychain and in the Android Keystore, the pinned peer
...
- The Android identity key is stored wrapped, with an AES-256-GCM key that cannot leave the
  ...
  here so it does not count as a finding on its own. A way to reach that memory from another app
  is a finding.
