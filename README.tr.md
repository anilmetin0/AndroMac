...
bildirim aynası, uygulama başına bildirim kademesi, telefonda çalan parça ve kontrolleri, bir de
kaybolan telefonu çaldıran düğme. Sunucu, hesap, bulut yok; iki tarafta da üçüncü parti kütüphane
...
| **Pano** | Telefondan Mac'e | Tek dokunuş: Hızlı Ayarlar karesi, kalıcı bildirimdeki düğme, paylaşım menüsü ya da uygulamanın kendisi. [Bu yön neden elle](#pano-neden-tek-yönde-elle-çalışıyor). |
...
| **Birden fazla telefon** | Mac | Aynı anda birden fazla Android eşleştirilip bağlanabilir. Panel bütün cihazları listeler, tıklayınca genişletir; her cihaz tek tek duraklatılıp sürdürülebilir — duraklatmak bağlantıyı da keser — ve her birinin kendi pano anahtarı vardır. Eşleştirme kaldırma Ayarlar → Cihazlar'da. |
...
### Pano neden tek yönde elle çalışıyor

Android 10'dan beri bir uygulama, ön planda değilse panoyu okuyamıyor. Bu kasıtlı bir gizlilik
kararı ve desteklenen bir baypası yok; bu yüzden telefondan Mac'e yön kullanıcı tetikli. Paylaşım
menüsü yolu panoya hiç dokunmaz, dördü içinde en temizi odur.
...
2. AndroMac'i aç. **Kurulum** bölümü ihtiyacı olanları ister:
...
3. Android 13 ve üstünde, dışarıdan kurulan uygulamalarda bildirim erişimi anahtarı gri görünür
   ...
   tekrar dene.
4. Her şey verilince Kurulum bölümü kendiliğinden kaybolur.
...
Sen açmadıkça iki uygulama da güncelleme aramaz. İki tarafta da **Ayarlar → Güncellemeler**'de
anahtar ve **Şimdi denetle** düğmesi var. Açıkken uygulama, yalnızca açılışta ve menü çubuğu paneli açıldığında, günde en fazla
bir kez, GitHub yayın API'sine en yeni yayını sorar. Daha yeni bir paket varsa, ister daha büyük
bir sürüm ister aynı sürümün başka bir commit'ten derlenmiş hali olsun, `1.0.0 (fd7d47a)`
biçiminde sürümü ve commit'i ile yayın sayfasına bağlantıyı gösteren bir satır çıkar. Kendi
başına hiçbir şey indirmez ya da kurmaz. Uygulamaların bir sunucuyla konuşmasına yol açan tek
şey bu anahtardır, bkz. [Gizlilik ve güvenlik](#gizlilik-ve-güvenlik).
...
| Güncellemeler | İsteğe bağlı güncelleme denetimi: anahtar, **Şimdi denetle** ve yeni paket varsa indirme bağlantısı. |
...
Mac'te menü çubuğu paneli göz atmak içindir. Sırayla şunları gösterir: eşleşmiş her telefonun kartı —
her satır genişletilebilir, o telefonun çalan parçasını, pano anahtarını, çaldırma düğmesini ve
Duraklat'ı taşır — altında pil, sonra son pano kaydı ve iki yönden birinde dosya giderken bir
ilerleme satırı, en son dört bildirim ve dört senkronizasyon anahtarı. Durum satırı yalnızca hiç
eşleştirme yokken ya da dinleyici kapalıyken en üstte çıkar. Panelin üstüne bırakılan dosyalar gönderilir. Ayrıntı pencerede.
...
| Ayarlar | Genel, Pano, Dosyalar, Güncellemeler, Cihazlar, Ağ, Ölçümler ve Gizlilik. |
...
değiştirince bir "Yeniden başlat" düğmesi çıkar. Güncellemeler, [Güncelleme
denetimi](#güncelleme-denetimi) başlığındaki isteğe bağlı denetimi barındırır. Dosyalar'da
**Dosya al** ve eşleşmiş bütün telefonlar için geçerli **Dosyaları otomatik kabul et** var. Pano
bölümünde eşitleme anahtarı, **Mac'ten telefona** için otomatik/elle seçimi ve gizli pano kuralı
var. Güncellemeler ayrıca sürüm sayfasının QR kodunu çizer; APK'yı telefona kurmak için adres
...
analitik yok. Uygulamalar internete hiçbir soket açmaz; tek istisna isteğe bağlı: Ayarlar →
Güncellemeler'deki, varsayılan olarak kapalı güncelleme denetimi. O da `api.github.com`'a günde
en fazla bir kez en yeni sürümü sorar ve `User-Agent` başlığındaki uygulama sürümünden başka
...
android/                           AGP 9.4.0, Gradle 9.7.1, minSdk 29, bağımlılık yok
  ...
  app/src/main/kotlin/dev/andromac/
    ...
    feature/UpdateCheck.kt         GitHub API'ye isteğe bağlı sürüm denetimi
...
macos/                             Swift paketi, swift-tools 6.2, macOS 14+, bağımlılık yok
  ...
  Sources/AndroMac/
    ...
    UpdateCheck.swift              GitHub API'ye isteğe bağlı sürüm denetimi
    ...
    DeviceList.swift               eşleşmiş her telefon; genişletilebilir, pano anahtarı ve Duraklat ile
...
| Pano telefondan kendiliğinden gelmiyor | Yukarıda anlatılan Android kısıtı. Kareyi, bildirimdeki düğmeyi ya da paylaşım menüsünü kullan. |
