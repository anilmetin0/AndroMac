...
The crypto is implemented twice and independently: CryptoKit on macOS, JCE on Android. One byte
of disagreement, in the X9.62 encoding, the HKDF info string, the nonce layout or the GCM tag
position, breaks the handshake on device with no useful error. The two scripts prove that the
...
the `dh2` and `dh3` order was written the wrong way round on macOS. CI runs both on every push,
on every branch.

## Rules

Keep the energy contract. The rules in [docs/ENERGY.md](docs/ENERGY.md) are part of the
protocol. No periodic timer on the phone, no polling, no wakelock, and filtering on the phone
before anything is sent. Expensive work belongs on the Mac, which is on wall power. If a change
needs a new wakeup on the phone, say why in the pull request description.
...
Validate what arrives. A sender's limits do not protect the receiver. Every incoming field has
its own length check on the receiving side, and new message fields need one too.
...
2. Translate the values only. Keep every `name` key, every placeholder such as `%1$s` and `%d`,
   and every plural form as they are, or the format calls will crash at runtime.
...
The in-app language picker lists whatever it finds in the bundle, so no further step is needed.
