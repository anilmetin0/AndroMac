...
/// The numbers of PROTOCOL §5: 512 KiB chunks, up to 8 in flight, files up to 4 GiB.
public enum FileTransferMath {

    public static let chunkSize = 524_288
    public static let maxFileSize: Int64 = 4 << 30

    /// How many chunks may be on the wire before an ack comes back.
    ///
    /// It used to be one, which made every 512 KiB cost a full round trip: on a home Wi-Fi network
    /// with 15 ms between the two devices that is a hard ceiling of about 30 MB/s no matter how
    /// fast the link is, and most of the time both radios sat idle waiting for each other. Eight
    /// keeps 4 MiB in flight, which fills a normal Wi-Fi link, and still bounds what an unacked
    /// sender can push at a phone. The receiver is unchanged: chunks are written strictly in order,
    /// which TCP already guarantees, and every one of them is still acked.
