...
///
/// There is a single rolling release per version: tag `v1.0.0`, title `AndroMac 1.0.0`. The tag
/// carries the version, the tag's target the commit, and the same version is re-published with a
/// new commit on every push. Pure data, so it can be unit-tested; the network side lives in
...
public struct Release: Equatable, Sendable {
    ...
    /// The macOS build of this release, `AndroMac-<version>-macOS-arm64.dmg`.
    public var macImage: Asset? { assets.first { $0.name.hasPrefix("AndroMac-") && $0.name.hasSuffix(".dmg") } }
    ...
    /// The `(fd7d47a)` part of a release title, how releases before 1.0.0 named their commit.
