...
/// takes about a millisecond, and the protocol keeps at most `FileTransferMath.window` of them in
/// flight, so the panel stays responsive. Every message goes out through `Server.send`, so the link counters see
...
final class FileTransfer: ObservableObject {
    ...
    private final class Outgoing {
        ...
        var nextSeq = 0
        /// The next sequence number an ack is expected for; `nextSeq - acked` is what is in flight.
        var acked = 0
        var sent: Int64 = 0
    ...
    /// Acks arrive in order and free one slot in the window each (§5 step 3).
    private func acked(_ id: String, _ msg: [String: Any]) async {
        guard let out = outgoing, out.id == id, out.phase != .offered else { return }
        guard msg["seq"] as? Int == out.acked else {
    ...
    /// Keep the window full: chunks go out until `FileTransferMath.window` of them are unacked
    /// (§5 step 3). The hash is computed while streaming so the file is read once (§6.12).
    private func pump() async {
        guard let out = outgoing, out.phase == .streaming else { return }
        while out.sent < out.size, out.nextSeq - out.acked < FileTransferMath.window {
        ...
        // `file_done` follows the last chunk on the same connection, so it cannot overtake it.
    ...
    ///
    /// The banner carries a preview of what arrived — the photo itself, the first page of the PDF,
    /// whatever Quick Look would draw in Finder. "File received" plus a file name is the one thing
    /// the Mac already knew; the picture is the part that tells the user the right file came.
    private func notify(id: String, file: URL) {
        Task {
            let preview = await Self.preview(of: file)
            let content = UNMutableNotificationContent()
            content.title = String(localized: "File received")
            content.body = file.lastPathComponent
            content.subtitle = ByteCountFormatter.string(
                fromByteCount: Int64((try? file.resourceValues(forKeys: [.fileSizeKey]))?.fileSize ?? 0),
                countStyle: .file
            )
            content.sound = .default
            content.userInfo = ["file": file.path]
            if let preview, let attachment = try? UNNotificationAttachment(identifier: "preview", url: preview) {
                // UserNotifications moves the file into its own store, so the temporary copy is
                // gone from our directory either way.
    ...
    /// A Quick Look thumbnail of the received file, as a PNG in a temporary directory, or nil when
    /// the type has no preview (an archive, a binary). The original file is only read, never moved.
    private static func preview(of file: URL) async -> URL? {
        let request = QLThumbnailGenerator.Request(
            fileAt: file, size: CGSize(width: 512, height: 512), scale: 2,
        ...
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("andromac-preview-\(id(from: file)).png")
        do {
    ...
    /// A stable, path-free name for the temporary preview: the file name itself may contain
    /// anything, and it does not belong in a path we build.
    private static func id(from file: URL) -> String {
        SHA256.hash(data: Data(file.path.utf8)).prefix(8).map { String(format: "%02x", $0) }.joined()
    }
