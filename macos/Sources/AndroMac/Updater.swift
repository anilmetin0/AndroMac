...
/// version" has to be followed by something the user can press. This is that button: fetch the zip
/// the release publishes, check it against the release's own `SHA256SUMS.txt`, unpack it, and swap
/// the bundle.
///
/// What it will not do:
...
final class Updater: ObservableObject {
    ...
    func install(_ release: Release) async {
        ...
        guard let zip = release.macZip, let sums = release.checksums else {
        ...
        do {
            phase = .downloading
            let archive = try await download(zip)
            staging = archive.deletingLastPathComponent()

            phase = .verifying
            let expected = try await checksum(named: zip.name, from: sums)
    ...
    /// `ditto` is the only unpacker that keeps a bundle's symlinks and extended attributes intact,
    /// which is why the release is built with it in the first place.
    private func unpack(_ archive: URL) throws -> URL {
        let destination = archive.deletingLastPathComponent().appendingPathComponent("unpacked")
        ...
        guard ditto.terminationStatus == 0 else {
            throw UpdateError.message(String(localized: "The download could not be unpacked."))
        }
        return destination
    }
