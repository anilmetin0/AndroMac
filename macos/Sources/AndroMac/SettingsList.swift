...
struct SettingsList: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Clipboard") {
                Toggle("Sync clipboard", isOn: $syncClipboard)
                ...
                Text("Android does not let a phone send its clipboard on its own: tap Send clipboard there.")
                    .font(Theme.Font.label)
            ...
            Section("Updates") {
                ...
                HStack {
                    ...
                    if let release = updates.available {
                        Button(String(localized: "Download \(release.label)")) {
                            NSWorkspace.shared.open(release.url)
                        }
                        .keyboardShortcut(.defaultAction)
                    }
                }
                Text("Asks api.github.com once a day for the newest release, sending the app version and nothing else. Off by default.")
                    .font(Theme.Font.label)
                    .foregroundStyle(.tertiary)
                    .fixedSize(horizontal: false, vertical: true)
