...
/// The menu bar panel: made for a glance. Current status, one-tap toggles, the last few
/// notifications. Search, long lists and rarely used settings live in a separate window ([MainWindow]).
struct MenuPanel: View {
    ...
    /// Which device rows the user has opened. Only meaningful with more than one phone.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: Theme.Space.small) {
            ...
            if isPaired {
                PanelCard {
                    DeviceList(expanded: $expandedDevices)
    ...
    /// Only while a newer release is known. Clicking opens its page; the panel does no downloading.
        ...
        Button {
            ...
            HStack(spacing: 8) {
                Image(systemName: "arrow.down.circle")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
                Text("AndroMac \(release.label) is available")
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
                    .lineLimit(1)
                Spacer(minLength: 4)
                ...
                HStack(spacing: 8) {
                    GeometryReader { geo in
                        ZStack(alignment: .leading) {
                            ...
                            Capsule()
                                .fill(batteryTint(b))
                                .frame(width: max(4, geo.size.width * CGFloat(b.level) / 100))
                                .animation(.easeOut(duration: 0.22), value: b.level)
                        }
                    }
                    .frame(height: 6)
    ...
    private func batteryTint(_ b: AppState.Battery) -> Color {
        if b.charging { return .green }
        switch b.level {
        case ..<15: return .red
    ...
    private func batteryDetail(_ b: AppState.Battery) -> String {
        var parts = [batteryLabel(b.status)]
        if let t = b.temperature { parts.append(String(format: "%.0f°C", t)) }
        return parts.joined(separator: " · ")
    }

    private func batteryLabel(_ status: String) -> String {
        switch status {
    ...
    private var clipboard: some View {
        HStack(spacing: 8) {
            ...
            if canSendClipboard { sendClipboardButton }
        }
    }
    ...
    private var canSendClipboard: Bool {
        state.isConnected && state.peerCaps.contains("clipboard") && syncClipboard
    }
    ...
    private var toggles: some View {
        // The switches are aligned to the right edge rather than next to THE LABEL: because the
        // labels differ in length (Clipboard / Battery / Notifications), the built-in Toggle
        // alignment puts all three at a different x and the panel looked ragged. It also makes the
        // whole row clickable.
        VStack(alignment: .leading, spacing: 6) {
            toggleRow(String(localized: "Battery"), systemImage: "battery.100", isOn: $syncBattery) {
                Store.shared.syncBattery = $0
            }
            toggleRow(String(localized: "Clipboard"), systemImage: "doc.on.clipboard", isOn: $syncClipboard) {
                Store.shared.syncClipboard = $0
            }
            toggleRow(String(localized: "Notifications"), systemImage: "bell", isOn: $syncNotifications) {
                Store.shared.syncNotifications = $0
            }
            toggleRow(String(localized: "Media"), systemImage: "music.note", isOn: $syncMedia) {
                Store.shared.syncMedia = $0
                // Drop the row immediately when switched off: the phone will send no more updates,
                // and leaving the old track on screen would show stale information.
                if !$0 { state.media = nil }
            }
        }
    }

    private func toggleRow(_ title: String, systemImage: String,
                           isOn: Binding<Bool>, onChange: @escaping (Bool) -> Void) -> some View {
        HStack(spacing: 8) {
            // The tap target is ONLY the label area: putting the gesture on the whole row means a
            // direct press on the switch fires both the control and the gesture, which can undo the
            // change. A separate label target → the two never collide and the row is still easy to hit.
            HStack(spacing: 8) {
