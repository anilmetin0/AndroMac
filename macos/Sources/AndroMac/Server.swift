...
actor Server {
    ...
    func stop(updateUI: Bool = true) async {
        ...
        sessions.removeAll()
        await MainActor.run { AppState.shared.devices.removeAll() }
        listener?.cancel(); listener = nil
        if updateUI { await MainActor.run { AppState.shared.status = .stopped } }
    }
    ...
    /// flag resets when the phone is plugged in or rises above 20%, otherwise it would repeat on
    /// every `battery` message.
    private func checkLowBattery(_ b: AppState.Battery, from deviceID: String) async {
        if b.charging || b.level > 20 { lowBatteryAlerted.remove(deviceID) }
        guard Store.shared.lowBatteryAlert, !lowBatteryAlerted.contains(deviceID),
              !b.charging, b.level <= 15 else { return }
