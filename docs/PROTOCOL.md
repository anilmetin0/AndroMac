...
the Mac watches its own pasteboard (§6) and sends on change. Android → macOS is never automatic,
because since Android 10 an app that is not focused and is not the active keyboard cannot read the
clipboard at all. The phone therefore sends when asked: its Quick Settings tile, the share sheet,
the in-app button — or `clipboard_request` below.
...
The phone cannot answer from its service, so it raises the invisible helper activity that takes
focus, reads the clipboard there and replies with `clipboard` — that is the whole trick, and it
needs "Display over other apps" on the phone, the one exemption an ordinary app can hold from the
background-activity-start rule. Without that permission the phone posts a notification with a
single "Send clipboard" button instead. **A phone that has granted neither answers nothing**, and
there is no path in this protocol that reads a phone's clipboard without one of them.
...
3. S streams `file_chunk` with a **window of 8**: up to eight chunks may be unacked at once,
   ...
   frame cap of §4, so 8 in flight is 4 MiB — bounded, because neither side has a
   back-pressure signal from its socket writer (§6) and an unbounded queue would grow the
   heap by the size of the file. The window used to be 1, which cost a full round trip per
   512 KiB: on a home network that is a ceiling of a few tens of MB/s regardless of how fast
   the link is, with both radios idle in between. R writes each chunk to a temporary location
   (macOS: `.<name>.part` next to the final file; Android: a `MediaStore` Downloads entry with
   `IS_PENDING=1`) and hashes it incrementally.
4. S sends `file_done` with the SHA-256 of the whole file right after the last chunk — same
   connection, so it cannot overtake it — computed while streaming (the file is read once;
   hashing before the offer would read a 1 GB video twice on battery). For `size=0` S sends
...
12. **File transfer is ack-driven, not timer-driven.** Chunks go out until eight are unacked, and
    each `file_ack` releases the next one; nobody polls for progress, and the phone sets no
    transfer timeout — a stalled transfer ends with the session (the Mac's `ping` catches a dead
    link). Progress UI on the phone updates at most once per second, decided by comparing
    timestamps when an ack arrives, never by a ticking timer. The hash is computed while streaming
