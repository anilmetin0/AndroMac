...
| Title | `AndroMac <VERSION> (<commit>)`, e.g. `AndroMac 1.0.0 (fd7d47a)` |
| Assets | `AndroMac-<VERSION>-<commit>-macOS.zip`, `AndroMac-<VERSION>-<commit>.apk`, `SHA256SUMS.txt` |
...
- `Casks/andromac.rb` uses `version :latest` and finds the current zip on the latest release
  through the GitHub API, so it needs no edit when a build or a version changes. The checksum is
  not pinned (`sha256 :no_check`) because every push changes the zip; `SHA256SUMS.txt` on the
