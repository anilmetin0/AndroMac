...
| Pano | Mac'ten telefona | Kopyaladıkça otomatik (Ayarlar → Pano) ya da pano geçmişindeki Telefona gönder ile elle. Elle gönderim telefona "Yapıştır" eylemli sessiz bir bildirim olarak düşer; arka planda panoya yazmayı engelleyen üreticiler için. Parola yöneticisinin gizli işaretlediği pano hiç gönderilmez. |
...
| Doğrulama kodları | Mac | Aynalanan bildirimde tek kullanımlık kod varsa Mac bildiriminde Kodu kopyala çıkar, panel de kodu bir kopyalama düğmesinin üstünde gösterir. Kod Android'in kendi kurallarıyla (AOSP'nin OTP algılayıcısı) bulunur: yanında kod, şifre, OTP ya da code gibi bir sözcük olmalı. Kopyalamak kodu telefona geri göndermez ve pano geçmişine yazmaz. |
