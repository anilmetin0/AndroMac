...
| 4 | **The mDNS browse is off while connected.** Continuous multicast browsing costs measurable battery. Browsing runs only when there is no connection, and for at most 8 s; a paired phone stops 1 s after its own Mac answers, so a same-name neighbour is seen too. | `Discovery.kt` / `LinkService.PAIRED_SETTLE_MS` |
