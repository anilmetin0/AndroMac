...
| # | Rule | Where |
|---|---|---|
| 1 | No periodic timer on the phone at all. The Mac does the liveness check (a `ping` every 240 s), and the phone only answers. | `Server.swift` pingTask / `LinkService.READ_TIMEOUT_MS` |
