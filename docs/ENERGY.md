...
| 13 | While disconnected, only the **cheap cached-address SYN runs every cycle**. The 8 s mDNS browse runs on the first attempt after a network event or a pairing request, then on every 4th attempt — and every cycle only while no address is cached, where nothing else can make progress. Repeated Wi-Fi flaps inside 10 s no longer restart the backoff ladder. | `LinkService.dial` |
