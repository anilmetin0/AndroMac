...
- **New app identifier: `io.github.anilmetin0.andromac`** on both platforms, replacing
  `dev.andromac`, a domain the project does not own. On Android this is a new app: install it,
  pair it again, then uninstall the old AndroMac. On the Mac the settings and paired phones carry
  over by themselves, but the 1.0 updater only accepts `dev.andromac`, so install 1.1.0 from the
  DMG or with Homebrew once.
