...
- **Clipboard, phone to Mac.** One tap from the Quick Settings tile, the button on the ongoing
  notification, the Android share sheet, or the app itself. Android does not allow background
  clipboard reads, so this direction is deliberately user-triggered.
...
- **File transfer.** Share any file from the phone's share sheet, or use Send file… and
  ...
  512 KiB chunks over the existing encrypted session with window-1 flow control, verified
  ...
  apps.
- **Update check.** Off by default on both apps. Settings → Updates has the switch and Check now.
  When on, the app asks the GitHub releases API at most once a day, only when it is opened, and
  shows the version and commit of the newer build with a link to it, whether that is a greater
  version or the same version built from a different commit. Nothing is downloaded or installed
  automatically.
