...
object FileTransfer {
    ...
    private fun onAccept(ctx: Context, id: String) {
        ...
        if (o.size == 0L) sendDone(ctx, o) else sendChunk(ctx, o)
    }

    private fun onAck(ctx: Context, id: String, seq: Int) {
        val o = out?.takeIf { it.id == id } ?: return
        if (seq != o.seq - 1) { cancelOut(ctx, "out_of_order"); return }
        if (o.sent >= o.size) sendDone(ctx, o) else sendChunk(ctx, o)
        val now = SystemClock.elapsedRealtime()
        if (out === o && now - o.lastUi >= UI_INTERVAL_MS) { o.lastUi = now; showOutProgress(ctx, o) }
    }

    private fun sendChunk(ctx: Context, o: Outgoing) {
        ...
        if (!Link.send(Protocol.fileChunk(o.id, o.seq, data))) { failOut(ctx, R.string.file_failed_send); return }
        o.seq++
    }
